...
#     val = self.client.get("active_proxies_cache")
#     return json.loads(val) if val else None


# def set_active_proxy_cache(self, proxies, ttl: int = 300):
#     self.ensure_connection()
#     if not self.client:
#         return

#     self.client.setex("active_proxies_cache", ttl, json.dumps(proxies))
...
#     self.client.setex(f"proxy_cd:{proxy_id}", duration, "1")
...
#     return self.client.exists(f"proxy_cd:{proxy_id}") > 0
...
class RedisClient:
    ...
    def set_task_state(self, task_id: str, task_state: Dict[str, Any], ttl: int = None):
        try:
            ...
            ttl = ttl or settings.task_ttl_seconds
            self.client.setex(f"task:{task_id}", ttl, json.dumps(task_state, default=str))
    ...
    def get_active_tasks(self, include_completed=False):
        try:
            ...
            tasks = {}
            for key in self.client.keys("task:*"):
    ...
    # ============================================================
    # 🟢 PROXY CACHE (FIXED - INSIDE CLASS)
    # ============================================================
    def get_active_proxy_cache(self):
        try:
            self.ensure_connection()
            ...
            logger.error(f"Proxy cache get error: {e}")
            return None
    ...
    def set_active_proxy_cache(self, proxies, ttl: int = 300):
        try:
            ...
            self.client.setex(
                "active_proxies_cache",
                ttl,
                json.dumps(proxies)
            )

        except Exception as e:
            logger.error(f"Proxy cache set error: {e}")
