...
class RabbitMQClient:
    ...
    async def connect(self):
        """Establish RabbitMQ connection (PRODUCTION SAFE)"""
        try:
            # ✅ ALWAYS use direct env (no fallback to localhost)
            rabbitmq_url = os.getenv("RABBITMQ_URL")

            if not rabbitmq_url:
                raise ValueError("RABBITMQ_URL is not set in environment variables")

            # ✅ Robust connection (auto-reconnect enabled)
            self.connection = await aio_pika.connect_robust(
                rabbitmq_url,
            ...
            logger.error(f"❌ Failed to connect to RabbitMQ: {e}")
            # ❗ IMPORTANT: DO NOT crash app in production
    ...
    async def publish_task(self, task: DownloadTask):
        ...
        try:
            ...
            if not self.exchange:
                raise Exception("RabbitMQ exchange not initialized")
